...
Готового брендбука на старте нет. Визуальный паттерн виджета (плавающая кнопка → панель чата, размеры, отступы) зафиксирован в `team/DECISIONS.md` (D-005) и реализован в `widget/src/widget.css`. Дефолтная цветовая схема и шрифт — тема сайта pravoe-delo.su (`team/DECISIONS.md` D-006/D-007, детали — `team/design/design-system.md`). Позиция и основной цвет настраиваются через `data-*` атрибуты loader-скрипта (`data-position`, `data-primary-color`) — детали дизайна здесь не дублируются.
